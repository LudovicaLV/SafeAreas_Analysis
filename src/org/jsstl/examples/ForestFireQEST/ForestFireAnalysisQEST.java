...
//	for (int j=0; j < meanTraj[0].length; j++){
//	for (int i=0; i < meanTraj.length; i++){		
//		SpatialThreeValues formula1 = SpatialThreeValuesTransducer.somewhere(TimeTSLValues1.get(j), 1.0, 4.0);
...
//	SpatialThreeValues formula1 = SpatialThreeValuesTransducer.not(resultFire1);
	...
	for (int j=0; j < meanTraj[0].length; j++){
	for (int i=0; i < meanTraj.length; i++){		
		SpatialThreeValues formula2 = SpatialThreeValuesTransducer.everywhere(TimeTSLValues1.get(j), 1.0, 1.0);
...
//	}
	...
	for (int i=0; i< graph.getNumberOfLocations(); i++){
		for (int j=0; j<Mapformula2.size();j++) {
		if (Mapformula2.get(j).spatialThreeValues.get(graph.getLocation(i))==ThreeValues.FALSE){
			text += String.format(Locale.US, " %20.10f", 0.2);}else{
				if(Mapformula2.get(j).spatialThreeValues.get(graph.getLocation(i))==ThreeValues.TRUE){
...
PrintWriter printer = new PrintWriter("data/EverywhereNew.txt");
printer.print(text);
printer.close();
